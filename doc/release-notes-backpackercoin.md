Backpackercoin Core version *0.15.1* is now available from:

  <https://download.backpackercoin.com/backpackercoin-0.15.1/>
...
A number of changes to the way Backpackercoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 milliBPC per kB (0.00001 BPC). This is to prepare for dropping the minimum transaction fee to 0.1 milliBPC per kB (0.0001 BPC) in the next release.
